"""
End to end workflow testing. This take a bit longer to run. In a production setting this would
be a slow test that does not run in the premerge tests but every say a few hours from master branch.

Also you would like to make all the possible configuration combinations and pass them to the 
@parameterize decorator above. Since this is for demostration purposes only I will only made two cases.
...
def test_end_to_end_workflow(benchmark, local_artifactory: Generator):
    ...
    with local_artifactory as artifactory:
        ...
        assert hasattr(
            model, "predict"
        ), "Trainded model does not have a predict method"
        ...
        assert (
            type(intent) == list
        ), f"model return type is {type(intent)} should be list"
        ...
        assert (
            "label" in intent[0].keys()
        ), f"model prediction does not have 'label' field"

        assert (
            "confidence" in intent[0].keys()
        ), f"model prediction does not have 'confidence' field"

        assert (
            type(intent[0]["label"]) == str
        ), f"model prediction 'label' is not a string"

        assert isinstance(
            intent[0]["confidence"], Number
        ), f"model prediction 'confidence' is not a number"
